...
### Armada Mermaid Theme

When using the Armada aesthetic with Mermaid diagrams, use these `themeVariables` to match the SailPoint design system. Full details in `./armada-tokens.md`.

```javascript
themeVariables: {
  primaryColor: isDark ? '#1e3a5f' : '#bcd1ff',     // p3 light
  primaryBorderColor: isDark ? '#5b8def' : '#1d5ade', // p3
  primaryTextColor: isDark ? '#e6edf3' : '#212b36',   // g4
  secondaryColor: isDark ? '#1a2e1a' : '#b3dbc5',     // s2 light
  secondaryBorderColor: isDark ? '#4ade80' : '#00883f', // s2
  tertiaryColor: isDark ? '#2e2510' : '#ffe5b4',      // s5 light
  tertiaryBorderColor: isDark ? '#fbbf24' : '#ffa719', // s5
  lineColor: isDark ? '#8b9bb3' : '#637381',           // g3
...
|---|---|---|
| **Source Sans 3** | **Atkinson Hyperlegible Mono** | **Armada / SailPoint** — use for internal SailPoint pages |
...
When using the Armada aesthetic, always pair Source Sans 3 with Atkinson Hyperlegible Mono. Optionally use Poppins for display headings. See `./armada-tokens.md` for Google Fonts import snippets.
