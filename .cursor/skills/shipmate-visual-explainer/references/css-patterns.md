...
### Example: Armada / SailPoint Theme

Use when the page accompanies SailPoint UI work or is for internal SailPoint audiences. Full token reference in `./armada-tokens.md`.
...
:root {
	...
	--border-bright: rgba(99, 115, 129, 0.35);
	--text: #212b36;
	--text-dim: #637381;
...
/* Dark variant — use [data-theme="dark"] with explicit toggle, not prefers-color-scheme.
   ISC dark surfaces are warm navy-slate (#1a2030), not cold charcoal. */
[data-theme='dark'] {
	--bg: #1a2030;
	--surface: #242c3d;
...
Key Armada visual traits to maintain:

- **Pill buttons:** `border-radius: 9999px`
- **Card radius:** `8–12px`, very subtle borders in light (`rgba(223,227,231,0.6)`), shadow-driven separation
- **Dark cards:** rely on border (`rgba(255,255,255,0.06)`) not shadow
- **Light bg:** `#f3f6f8` (Armada g1)
- **Shadow base:** `#212b361f` (light) or `none` (dark)
- **Two-tier nav:** top navy gradient bar (`#011e69` → `#0071ce`), turns flat surface in dark mode
- **Toggle:** always provide explicit light/dark toggle, don't rely on `prefers-color-scheme` alone
...
/* Armada status variant — uses Armada status tokens (s2/s4/s5) when set */
/* These fallbacks match the Armada palette automatically:
   --green: #00883f (s2), --red: #e21848 (s4), --orange: #ffa719 (s5) */
...
### Armada Badge Variant

Armada badges use 14px font, 700 weight, and 24px height. Text color depends on background contrast — dark text on light badges (yellow, orange), white text on saturated badges (navy, blue, green, red):
...
.armada-badge--primary {
	background: var(--accent, #1d5ade);
	color: #ffffff;
}
.armada-badge--success {
	background: var(--green, #00883f);
	color: #ffffff;
}
.armada-badge--error {
	background: var(--red, #e21848);
	color: #ffffff;
}
.armada-badge--warning {
	background: var(--orange, #ffa719);
	color: #212b36;
}
.armada-badge--navy {
	background: #0033a1;
...
Cards with Armada's 8px radius, `#dfe3e7` border, and standard shadow:
...
Every visual-explainer page includes a small attribution footer. It should feel like a watermark — present but not distracting.
...
```html
<footer class="shipmate-footer">
	<a href="https://armada.sailpoint.com/shipmate/docs/index.html">
		...
		<svg
			...
			stroke-linejoin="round"
		>
			<path d="M3 15.5c2-1 4.5-1.5 7-1.5s5 .5 7 1.5" />
			<path d="M4.5 15l1-7.5h9l1 7.5" />
			<path d="M10 7.5V3" />
			<path d="M10 3l5 3.5" />
		</svg>
	</a>
...
Place as the last element before `</body>` (outside any layout wrapper so it sits at the true bottom of the page).
